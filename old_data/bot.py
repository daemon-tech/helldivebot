...
'''
print("DEBUG: loop task update owner [alert log]")
@tasks.loop(minutes=6)
async def alert(channel_id):
    status = alert_for_conquered_planet() # -> output -> message
    channel = bot.get_channel(channel_id)
    if channel is None:
        print(f"Error: Channel with ID {channel_id} not found.")
        return
        ...
        #logic embed
    embed = discord.Embed(title=":ringed_planet: Alert :ringed_planet:", color=discord.Color.blue())
    embed.add_field(name=f"Alert! -> Ownership change!", value=f"{status}", inline=True)
    await channel.send(embed=embed)
    print(f"Console: send.message embed (alert)")
'''
